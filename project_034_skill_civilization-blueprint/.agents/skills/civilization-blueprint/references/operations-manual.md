...
手册版本 1.4。节点指一个具有目标、行动和认知变化的故事单元；本流程将其默认映射为一条15–20秒视频。更长事件拆成多个节点，不强行塞入一条短片。所有参数为制作目标，API参数以当前工具schema为准。
...
| 2 建立世界逻辑 | Codex文字推理；涉及真实历史时使用官方/学术来源检索 | brief.json与有效源文档 | 使用模板W；默认完整探索弧，章节数按内容；覆盖下列世界维度 | `01_world.md` | 昔日世界维度齐全；当前无人遗迹与弃置时间自洽；机制有边界和可见痕迹 |
| 3 剧情和逐镜规划 | Codex写作＋时间表核算；可用Python做秒数求和 | 世界MD、可复用实体列表 | 模板S；每节点15–20秒；默认3个镜头，按动作增减 | `02_story.md`、逐节点镜头表JSON | 时间无空档/重叠；证据先于推断；无当地NPC；未向玩家直接揭底 |
| 4 资产拆解与生成预检 | Codex清单处理＋imagegen工具发现；或已配置Image2技能/CLI帮助 | 剧本、旧图PNG/JPG、世界风格 | 内部实体引用、视图列表、真实参考文件；确认参考编辑、预算与输出位置 | `design_plan.json`、参考映射、缺项表 | 每镜Traveller/道具/遗迹及证据均有内部制作引用；复用图真实存在；收费条件与权限覆盖 |
...
| 参数 | 默认/填法 | 判断或限制 |
|---|---|---|
| 文明内容规模 | 覆盖地理生态、资源经济、治理阶层、日常生活、信仰习俗、技术边界、历史、地区路线、视觉、危险退路 | 不用字数替代完整性；每个主要地区写原用途/原使用者/原供给/材料/尺度/弃置后的遗留痕迹 |
...
```text
根据文明想法{idea}，创作{civilization_name}的内部完整设定。前提：探索当下已废弃且无文明居民。分别写昔日全貌、弃置过程、现在遗迹；昔日完整答案不得直接交给玩家。
...
```text
依据{world_snapshot}，让Traveller围绕{goal}勘察无人遗迹。不得加入当地NPC带路/递钥/解释历史。玩家靠传回资产拼凑历史，Traveller只报告有限观察和推测。
...
```text
生成{entity_ref}的{view_role}设定图，用于后续跨镜头一致性。当前遗迹已无人，表现{弃置时长}对应的材料与失效痕迹；不生成盛世居民或无来由的维护活动。
...
必须逐项填写passed/failed/unverified及证据：世界维度覆盖、弃置时长自洽、当下无文明居民、证据公平性、无内部答案泄露、机制自洽、剧情因果、知识揭示顺序、镜头秒数、空间连续、母图身份、图册文件、节点资产覆盖。任一必要项failed或unverified不能把整个handoff标passed。工具受限时交付明确的blocked准备稿。
...
| 历史形象参考（条件项） | 仅壁画/记录确需时 | 首次新增，以后复用 | 标historical_depiction；不作为当下角色出镜 |
...
本节与 [遗迹与证据契约](ruins-evidence.md) 共同约束前述七部分。主文档依然保持完整，新增证据计划是附属制作记录。

| 步骤 | 工具 | 额外输入 → 输出 | 参数／判断 | 失败返工 |
|---|---|---|---|---|
| 世界建构 | Codex写作与来源阅读 | 一句话 → 昔日内部全貌＋废弃过程＋当前遗迹 | present_civilization_population=0；完整真相author_only | 仍有人生活则重写当前社会为历史，推导现有物证 |
| 剧情拆解 | Codex＋时间核算 | 当前遗迹 → 每节点观察与传回资产 | 无向导NPC；observed与inferred分开 | 删除讲解者后补寻找路线/工具的可见依据，不能只删对白 |
| 证据编排 | Codex JSON编辑 | 核心命题 → evidence_plan.json | 主要推断以不同痕迹交叉支持；缺证则保留疑点 | 补证据镜或降低结论确定性 |
| 图册生成 | 内置imagegen／已授权Image2 | 当前遗迹母图与痕迹需求 → 实图 | 无当地活人；历史描绘与复原明确隔离 | 返修居民、过新表面、过量完整信息；保留rNNN |
| 交接检查 | 看图＋清单核对 | 图片／证据表 → handoff与QA | player_asset_refs明确白名单；不得含作者答案 | 内部复原移出传回清单，下游标stale重验 |

单节点还需：1份知识状态卡、实际证据引用、对应传回资产任务、支持／不支持结论、交叉证据与剧透限制。字段见节点模板；计划路径为空不代表实图已存在。

| 新增失败 | 返工方法 |
|---|---|
| 无人城仍有接待者、住区欢呼或守门人 | 去除当地当下角色，重建探索动机和物件获取过程 |
| 灯亮被直接解释为有居民 | 说明残留能源及故障周期，只传回观察，不推定人口 |
| 玩家拿到全知旁白或完整复原图 | 改为局部证据；复原与答案仅内部使用 |
| 文明完整性因“无人”被删光 | 补昔日生活制度，转译成遗迹差异、磨痕和残留物 |
| 同一段铭文成为所有结论的唯一来源 | 加入独立环境证据或承认不可判定 |
